...
static int refcompress(unsigned char *from, int len, unsigned char *dest, int hlen, int maxback, int quick)
{
    unsigned int tlen;
    unsigned int tcost;
//    unsigned int ccost;    // context cost
    ...
    while (len>=0)
    {
        ...
        bcost = 2;
//        ccost = 0;
		int beff = 0; // decode efficiency rating
        mlen = qmin(len,1028);
        tptr=cptr-1;
        hash = HASH(cptr);
            ...
            do
            {
                ...
                if (cptr[blen]==tptr[blen])
                {
                    ...
                    if (tlen > blen)
                    {
                        ...
                        {
							int teff = tlen;	// efficiency is a function of length and alignment
							if (((int)tptr&1)==0) ++teff;
							if (((int)tptr&3)==0) ++teff;
							if ((tlen-tcost > blen-bcost) || (teff>beff))
							{
	                            blen = tlen;
		                        bcost = tcost;
			                    boffset = toffset;
...
//        if (bcost>blen || (blen<=2 && bcost==blen && !ccost) || (len<4))
        if (bcost>=blen || len<4)
        {
			...
			if ((rptr-from)>(to-dest+overlap))
				overlap = (rptr-from)-(to-dest);
        }
        else
        {
    ...
    *to++ = (unsigned char) (0xfc+run); /* end of stream command + 0..3 literal */
	...
	overlap = (to-dest+overlap)-(rptr-from);
	gfree(link);
	gfree(hashtbl);
    return(to-dest);
}
...
int GCALL REF_encode(void *compresseddata, const void *source, int sourcesize, int *opts)
{
    ...
    return refcompress((unsigned char *)source, sourcesize, (unsigned char *)compresseddata, hlen, maxback, quick);
}

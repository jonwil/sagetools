...
ms_sparsing(lame_internal_flags* gfc, int gr)
{
    ...
    for ( sfb = gfc->l3_side.tt[gr][0].sfb_smin; sfb < SBPSY_s; ++sfb ) {
        ...
        treshold = pow( 10.0, -(FLOAT8)(gfc->sparseA-gfc->mld_s[sfb]*gfc->sparseB)/10.0);
...
{
    const FLOAT8 *a = (FLOAT8 *)v1, *b = (FLOAT8 *)v2;
    if (*a > *b) return 1;
    if (*a == *b) return 0;
    return -1;
}

...
/*	FLOAT fVersion = LAME_MAJOR_VERSION + 0.01 * LAME_MINOR_VERSION; */
	...
	if (gfp->findReplayGain) { 
	  ...
	  if (gfc->RadioGain >= 0) 
        ...
        if(gfc->findPeakSample)
          nPeakSignalAmplitude = abs((int)((((FLOAT8)gfc->PeakSample) / 32767.0 ) * pow(2.0f,23) +.5));
	...
	CreateI4((unsigned char *)&pbtStreamBuffer[nBytesWritten], nQuality);
	nBytesWritten+=4;
	...
	CreateI4((unsigned char *)&pbtStreamBuffer[nBytesWritten], nPeakSignalAmplitude);
	nBytesWritten+=4;

	CreateI2((unsigned char *)&pbtStreamBuffer[nBytesWritten],nRadioReplayGain);
	nBytesWritten+=2;

	CreateI2((unsigned char *)&pbtStreamBuffer[nBytesWritten],nAudiophileReplayGain);
	nBytesWritten+=2;
	...
	CreateI2((unsigned char *)&pbtStreamBuffer[nBytesWritten], gfp->preset);
	nBytesWritten+=2;

	CreateI4((unsigned char *)&pbtStreamBuffer[nBytesWritten], nMusicLength);
	nBytesWritten+=4;

	CreateI2((unsigned char *)&pbtStreamBuffer[nBytesWritten], nMusicCRC);
	nBytesWritten+=2;
	...
	CreateI2((unsigned char *)&pbtStreamBuffer[nBytesWritten], crc);
	nBytesWritten+=2;

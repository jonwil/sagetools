    ...
    )
{
    int sb, sblock;
    FLOAT msfix2 = msfix;
    FLOAT athlower = pow(10.0f, (FLOAT)athadjust);
...
{
    ma_max_i1 = pow(10.0,(FLOAT8)(I1LIMIT+1)/16.0);
    ma_max_i2 = pow(10.0,(FLOAT8)(I2LIMIT+1)/16.0);
    ma_max_m  = pow(10.0,(FLOAT8)(MLIMIT)/10.0);
}
...
int psymodel_init(lame_global_flags *gfp)
{
	...
	x = (-20.0+bval[i]*20.0/10.0);
	if (bval[i]>10) x = 0.0;
	gfc->minval[i]=pow(10.0,x/10.0);
...
#define  frame_duration (576. * gfc->mode_gr / sfreq)
    gfc->ATH->decay = pow(10., -12./10. * (FLOAT8)frame_duration);
...
#undef  frame_duration
	...
	for( i = 0; i < BLKSIZE/2; ++i ) {
	    ...
	    /*  to determine eql_w */
	    freq += freq_inc;
	    gfc->ATH->eql_w[i] = 1.0f / pow( 10.0f, ATHformula( freq, gfp ) / 10.0f );

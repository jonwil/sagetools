...
iteration_init( lame_global_flags *gfp)
{
  ...
  if ( gfc->iteration_init_init==0 ) {
...
#endif
    for (i = 0; i < Q_MAX; i++)
	ipow20[i] = pow(2.0, (FLOAT8)(i - 210) * -0.1875);
    for (i = 0; i < Q_MAX+Q_MAX2; i++)
	pow20[i] = pow(2.0, (FLOAT8)(i - 210 - Q_MAX2) * 0.25);
    for (i = 0; i < Q_MAX2; i++)
        iipow20[i] = pow(2.0, (FLOAT8)i * 0.1875);
    ...
    if (gfp->psymodel == PSY_NSPSYTUNE) {
        ...
        if (i >= 32)
            i -= 64;
        bass = pow(10.0, (FLOAT8)i / 4.0 / 10.0);

        i = (gfp->exp_nspsytune >> 8) & 63;
        if (i >= 32)
            i -= 64;
        alto = pow(10.0, (FLOAT8)i / 4.0 / 10.0);

        i = (gfp->exp_nspsytune >> 14) & 63;
        if (i >= 32)
            i -= 64;
        treble = pow(10.0, (FLOAT8)i / 4.0 / 10.0);
        ...
        if (i >= 32)
            i -= 64;
        sfb21 = treble * pow(10.0, (FLOAT8)i / 4.0 / 10.0);
